...
class WiFiNetworkScanner extends Component {
  ...
  scanNetworks() {
    ...
    fetch(SCAN_NETWORKS_ENDPOINT).then(response => {
      ...
      throw Error(<IntText text="WiFiSettings.ScanErrorAlert" /> + response.status);
    }).catch(error => {
  ...
  retryError() {
    return {
      ...
      message: <IntText text="WiFiSettings.NetWorkNotReady" /> + POLLING_FREQUENCY + "ms."
    };
  }
    ...
    fetch(LIST_NETWORKS_ENDPOINT)
      .then(response => {
        ...
        if (response.status === 202) {
          if (++this.pollCount < NUM_POLLS) {
            ...
            throw Error(<IntText text="WiFiSettings.DeviceNotReturn" />);
          }
        }
        throw Error(<IntText text="WiFiSettings.DeviceReturnError" /> + response.status);
      })
      .then(json => {
      ...
      .catch(error => {
        ...
        if (error.name !== RETRY_EXCEPTION_TYPE) {
          this.props.enqueueSnackbar(<IntText text="WiFiSettings.ProblemScanning" /> + error.message);
          this.setState({ scanningForNetworks: false, networkList: null, errorMessage: error.message });
        }
      });
  }
